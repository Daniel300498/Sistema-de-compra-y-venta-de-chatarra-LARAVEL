      ...
      <div class="col-lg-12">
        <div class="card">
          <div class="card-body">
            ...
            {!! Form::model($memorandums,['route'=>['memorandums.updated',$memorandums->id],'method'=>'PUT','class'=>'row g-3','enctype'=>"multipart/form-data"]) !!}
              @csrf
              <input type="hidden" name="memorandums_id" id="memorandums_id" value="{{ $memorandums->id }}" >
              <div class="row mb-3">
                  <div class="col-lg-3">
                    {{Form::label('fecha_emision','Fecha De Emision')}} <span class="text-danger">(*)</span>
                        <input id="fecha_emision" type="date" class="form-control {{ $errors->has('fecha_emision') ? ' error' : '' }}" name="fecha_emision" value="{{ $memorandums->fecha_emision }}"   >
                  ...
                  <div class="col-lg-3">
                    ...
                    <select name="tipo" id="tipo" class="form-control {{ $errors->has('tipo') ? ' error' : '' }}"  id="tipo" Onchange = "mostrar()" data-old="{{ old('tipo') }}">
                      ...
                      @foreach($tipos_memorandum as $tipo)
                        <option value="{{ $tipo->id}}" {{ old('tipo',$memorandums->memorandum_tipo_id)== $tipo->id? 'selected' :'' }}>{{ $tipo->descripcion }}</option> 
                      @endforeach
                    </select>
                  ...
                  @if ($memorandums->memorandum_tipo_id==6)
                  <div class="col-lg-3" >
                    <label for="todos_empleados" >Generado Por<span class="text-danger">(*)</span></label>
                    <select name="todos_empleados"  class="form-control form-control {{ $errors->has('todos_empleados') ? ' error' : '' }}" id="tipo_empleados" >
                      <option value="">--SELECCIONE--</option>
                      @foreach($empleados as $e)
                          <option value="{{$e->id}}" {{ old('todos_empleados',$memorandums->emitido_por) == $e->id ? 'selected' :'' }} >{{$e->nombres}} {{$e->ap_paterno}} {{$e->ap_materno}}</option>
                      @endforeach
                    </select>
                  </div>
                  @endif
                ...
                <div class="col-lg-8">
                  ...
                  <select name="cargo_id" id="cargo_id" class="form-control {{ $errors->has('cargo_id') ? ' error' : '' }}">
                    ...
                    @foreach ($cargos as $ca)
                        <option value="{{$ca->id}}" {{ old('cargo_id',$memorandums->cargo_id) == $ca->id ? 'selected' :'' }}>{{$ca->nro_item}} - {{$ca->nombre}}</option>
                    @endforeach
                  </select>
              ...
              <div class="row">
                <div class="col-lg-12">
                    ...
                    <textarea name="contenido" id="contenido" cols="100" rows="7" >{{$memorandums->contenido}}</textarea>
              ...
              <div class="text-center mt-3">
                  ...
                  <a href="{{ route('memorandums.create',$empleado->id) }}" class="btn btn-danger">Salir</a>
              </div>
