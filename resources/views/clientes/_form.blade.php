...
<div class="col-lg-6 mt-2">
    ...
    <select id="pais" class="form-control {{ $errors->has('pais') ? ' error' : '' }}" name="pais" value="{{ old('pais',$cliente->pais) }}"   onkeyup="javascript:this.value=this.value.toUpperCase();" onkeydown="return soloLetras(event);">
        ...
        @foreach($paises as $pais)
            <option value="{{ $pais->descripcion }}" {{ old('pais',$cliente->pais) == $pais->descripcion ? 'selected' : '' }}>
                {{ $pais->descripcion }}
            </option>
        @endforeach
    </select>
...
<div class="col-lg-4 mt-2">
    ...
    <input type="text" class="form-control {{ $errors->has('nit') ? ' error' : '' }}" name="nit" id="nit" value="{{ old('nit',$cliente->nit) }}"  onkeydown="javascript: return event.keyCode === 8 || event.keyCode === 9 ||
...
<div class="row mt-2">
    <div class="text-center">    
     <button type="submit" class="btn btn-primary">Guardar</button>
       <a href="{{ route('clientes.index') }}" class="btn btn-danger">Salir</a> 
    </div>
</div>
