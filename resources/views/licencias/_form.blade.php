...
<input type="hidden" name="area" id="area" value="{{$empleado->cargo[0]->pivot['cargos.area_id']}}">
    ...
    <div class="col-md-10">
        ...
        <div class="form-group d-flex align-items-center mb-2">
            <div class="col-md-5 text-right">
                {{Form::label('tipo_id','Tipo Licencia')}} <span class="text-danger">(*)</span> &nbsp;&nbsp;
            </div>
            
            <div class="col-md-7 ">
                <select name="tipo_id" class="form-control  {{ $errors->has('tipo_id') ? ' error' : '' }}" id="tipo_id" onchange="changeType()" >
                    <option value="">--   SELECCIONE   --</option>
                    @foreach ($tipoLicencia as $t)
                        @if ($t->descripcion!="VACACION")
                            @if ($t->descripcion=="CON GOCE")
                                @if($con_goce==true)
                                <option value="{{$t->id}}" {{ old('tipo_id',$licencia->tipo_id) == $t->id ? 'selected' : '' }} >
                                    {{$t->descripcion}} 
                                </option>
                                @endif
                            @else
        ...
        <div class="form-group d-flex align-items-center mb-2">
            <div class="col-md-5 text-right">
                {{Form::label('jefe_inmediato','Jefe Inmediato')}} <span class="text-danger">(*)</span>&nbsp;&nbsp;
            </div>
            <div class="col-md-7">
                <select name="jefe_inmediato" id="jefe_inmediato" class="form-control  {{ $errors->has('jefe_inmediato') ? ' error' : '' }}" >
                    <option value="">--   SELECCIONE   --</option>
                   @foreach ($empleados as $e)
                        <option value="{{$e->empleado->id}}" {{ old('jefe_inmediato',$licencia->jefe_inmediato) == $e->empleado->id ? 'selected' : '' }} >{{$e->empleado->nombres}} {{$e->empleado->ap_paterno}} {{$e->empleado->ap_materno}} - {{$e->cargo->nombre}}</option>
                    @endforeach
                </select>
                @if ($errors->has('jefe_inmediato'))                                                                                                        
                    <span class="text-danger">
        ...
        <div  name="div_estado_critico" id="div_estado_critico"  style="display: none;">
            <div class="form-group d-flex align-items-center mb-2">
                <div class="col-md-5 text-right">
                    {{Form::label('estado_critico','Condición o estado crítico de salud')}} <span class="text-danger">(*)</span>&nbsp;&nbsp;
                </div>
                <div class="col-md-7">
                    <select name="estado_critico" id="estado_critico" onchange="changeTypeEspecial();" class="form-control  {{ $errors->has('estado_critico') ? ' error' : '' }}">
                        <option id="tipo_nulo" name="tipo_nulo" value="">--   SELECCIONE   --</option>
                        @foreach ($enfermedad_licencia_especial as $enfermedad)
                            <option value="{{$enfermedad->id}}" {{old('estado_critico',$licencia->enfermedad_especial) == $enfermedad->id ? 'selected' : '' }}>{{$enfermedad->descripcion}}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('estado_critico'))                                                                                                        
                        <span class="text-danger">
                            {{ $errors->first('estado_critico') }}
                        </span>
                    @endif
                </div>
            </div>
        ...
        <div name="div_motivo_asueto" id="div_motivo_asueto"  style="display: none;">
            <div class="form-group d-flex align-items-center mb-2">
                <div class="col-md-5 text-right">
                    {{Form::label('motivo_asueto','Motivo asueto')}} <span class="text-danger">(*)</span>&nbsp;&nbsp;
                </div>
                <div class="col-md-7">
                    <select name="motivo_asueto" id="motivo_asueto" onchange="changeTypeMotivoAsueto();" class="form-control  {{ $errors->has('motivo_asueto') ? ' error' : '' }}">
                        <option id="tipo_nulo" name="tipo_nulo" value="">--   SELECCIONE   --</option>
                        @foreach ($tipoMotivoAsueto as $asueto)
                            <option value="{{$asueto->id}}" {{old('motivo_asueto',$licencia->tipo_motivo)  == $asueto->id ? 'selected' : '' }}>{{$asueto->descripcion}}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('motivo_asueto'))                                                                                                        
                        <span class="text-danger">
                            {{ $errors->first('motivo_asueto') }}
                        </span>
                    @endif
                </div>
            </div>
        ...
        <div name="div_motivo_baja" id="div_motivo_baja"  style="display: none;">
            <div class="form-group d-flex align-items-center mb-2">
                <div class="col-md-5 text-right">
                    {{Form::label('motivo_baja','Motivo Baja')}} <span class="text-danger">(*)</span>&nbsp;&nbsp;
                </div>
                <div class="col-md-7">
                    <select id="tipo_baja_medica" name="tipo_baja_medica" onchange="changeTypeMotivoBaja();" class="form-control  {{ $errors->has('tipo_baja_medica') ? ' error' : '' }}" >
                        <option id="tipo_nulo" name="tipo_nulo" value="">--   SELECCIONE   --</option>
                        @foreach ($tipo_baja_medica as $tipo_baja)
                            <option value="{{$tipo_baja->id}}" {{old('tipo_baja',$licencia->tipo_motivo) == $tipo_baja->id ? 'selected' : '' }}>{{$tipo_baja->descripcion}}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('tipo_baja_medica'))                                                                                                        
                        <span class="text-danger">
                            {{ $errors->first('tipo_baja_medica') }}
                        </span>
                    @endif
                </div>
            </div>
            ...
            <div class="form-group d-flex align-items-center mb-2">             
                ...
                <div class="col-md-7">
                    ...
                    <select id="tratamiento_cancer" name="tratamiento_cancer" onchange="changeTypeTratamientoCancer();" class="form-control " style="display: none; margin-bottom:10px;">
                        <option id="tipo_nulo" name="tipo_nulo" value="">--   SELECCIONE   --</option>
                        @foreach ($tratamiento_licencia_especial_cancer as $tratamiento_cancer)
                            <option value="{{$tratamiento_cancer->id}}" {{old('tratamiento_cancer',$licencia->tipo_motivo) == $tratamiento_cancer->id ? 'selected' : '' }}>{{$tratamiento_cancer->descripcion}}</option>
                        @endforeach
                    </select>
                    <select id="tratamiento_transplante" name="tratamiento_transplante" onchange="changeTypeTratamientoTransplante();" class="form-control " style="display: none; margin-bottom:10px;">
                        <option id="tipo_nulo" name="tipo_nulo" value="">--   SELECCIONE   --</option>
                        @foreach ($tratamiento_licencia_especial_transplante_otro as $tratamiento_transplante)
                            <option value="{{$tratamiento_transplante->id}}" {{old('tratamiento_transplante',$licencia->tipo_motivo) == $tratamiento_transplante->id ? 'selected' : '' }}>{{$tratamiento_transplante->descripcion}}</option>
                        @endforeach
                    </select>
                    <select id="tratamiento_insuficiencia" name="tratamiento_insuficiencia" onchange="changeTypeTratamientoInsuficiencia();" class="form-control " style="display: none; margin-bottom:10px;">
                        <option id="tipo_nulo" name="tipo_nulo" value="">--   SELECCIONE   --</option>
                        @foreach ($tratamiento_licencia_especial_insuficiencia as $tratamiento_insuficiencia)
                            <option value="{{$tratamiento_insuficiencia->id}}" {{old('tratamiento_insuficiencia',$licencia->tipo_motivo) == $tratamiento_insuficiencia->id ? 'selected' : '' }}>{{$tratamiento_insuficiencia->descripcion}}</option>
                        @endforeach
                    </select>
                    <select id="tratamiento_discapacidad" name="tratamiento_discapacidad" onchange="changeTypeTratamientoDiscapacidad();" class="form-control " style="display: none; margin-bottom:10px;">
                        <option id="tipo_nulo" name="tipo_nulo" value="">--   SELECCIONE   --</option>
                        @foreach ($tratamiento_licencia_especial_discapacidad as $tratamiento_discapacidad)
                            <option value="{{$tratamiento_discapacidad->id}}" {{old('tratamiento_discapacidad',$licencia->tipo_motivo) == $tratamiento_discapacidad->id ? 'selected' : '' }}>{{$tratamiento_discapacidad->descripcion}}</option>
                        @endforeach
                    </select>
                    <select id="tratamiento_accidente_muy_grave" name="tratamiento_accidente_muy_grave" onchange="changeTypeTratamientoAccidenteMuyGrave();"  class="form-control " style="display: none; margin-bottom:10px;">
                        <option id="tipo_nulo" name="tipo_nulo" value="">--   SELECCIONE   --</option>
                        @foreach ($tratamiento_licencia_especial_accidente_muy_grave as $tratamiento_accidente_muy_grave)
                            <option value="{{$tratamiento_accidente_muy_grave->id}}" {{old('tratamiento_accidente_muy_grave',$licencia->tipo_motivo) == $tratamiento_accidente_muy_grave->id ? 'selected' : '' }}>{{$tratamiento_accidente_muy_grave->descripcion}}</option>
                        @endforeach
                    </select>
                    <select id="tratamiento_accidente_grave" name="tratamiento_accidente_grave" onchange="changeTypeTratamientoAccidenteGrave();" class="form-control " style="display: none; margin-bottom:10px;">
                        <option id="tipo_nulo" name="tipo_nulo" value="">--   SELECCIONE   --</option>
                        @foreach ($tratamiento_licencia_especial_accidente_grave as $tratamiento_accidente_grave)
                            <option value="{{$tratamiento_accidente_grave->id}}" {{old('tratamiento_accidente_grave',$licencia->tipo_motivo) == $tratamiento_accidente_grave->id ? 'selected' : '' }}>{{$tratamiento_accidente_grave->descripcion}}</option>
                        @endforeach
                    </select>
    
                    <select  id="tipo_motivo" name="tipo_motivo" class="form-control "  onchange="changeTypeMotivo({{$licencia_rip[0]->dias}});" style="display: none;">
                        <option id="tipo_nulo" name="tipo_nulo" value="">--   SELECCIONE   --</option>
                        @foreach ($tipoMotivo as $t)
                            @if($licencia_rip[0]->dias < 2 )
                            <option value="{{$t->id}}" {{ old('tipo_motivo',$licencia->tipo_motivo) == $t->id ? 'selected' : '' }}>{{$t->descripcion}}</option>
                            @else
                                @if($t->descripcion!="RIP")
                                    <option value="{{$t->id}}" {{ old('tipo_motivo',$licencia->tipo_motivo) == $t->id ? 'selected' : '' }}>{{$t->descripcion}}</option>
                                @else
                                    @if($flag_rip == true)
                                        <option value="{{$t->id}}" {{ old('tipo_motivo',$licencia->tipo_motivo) == $t->id ? 'selected' : '' }}>{{$t->descripcion}}</option>
                    ...
                    <input id="motivo" name="motivo" type="text" class="form-control {{ $errors->has('motivo') ? ' error' : '' }}" value="{{ old('motivo',$licencia->motivo) }}" autofocus onkeyup="javascript:this.value=this.value.toUpperCase();" onkeydown="return soloLetras(event);" >
        ...
        <div class="form-group d-flex align-items-center mb-2">
            ...
            <div class="col-md-7">
                @if(Auth::user()->id != 4)
                    <input type="date" name="fecha_registro" id="fecha_registro" class="form-control {{ $errors->has('fecha_registro') ? ' error' : '' }}"  value="{{ old('fecha_registro',$licencia->fecha_registro)}}" >
                @else   
                    @if($licencia->fecha_registro==null)
                        <input type="hidden" name="fecha_registro" id="fecha_registro" class="form-control {{ $errors->has('fecha_registro') ? ' error' : '' }}"  value="<?php echo date("Y-m-d");?>" >    
                        <input type="date" class="form-control" value="<?php echo date("Y-m-d");?>" disabled >
                    @else
                        <input type="hidden" name="fecha_registro" id="fecha_registro" class="form-control {{ $errors->has('fecha_registro') ? ' error' : '' }}"  value="{{ old('fecha_registro',$licencia->fecha_registro)}}" >    
                        <input type="date" class="form-control" value="{{$licencia->fecha_registro}}" disabled >
                    @endif
                @endif
        ...
        <table class="table table-bordered" id="invoice-details" style="border-color: white; width:100%;" >
            ...
            <tbody>
                        ...
                        <td style="width: 30%">
                            <select id="dates[0][horario]" name="dates[0][horario]" class="form-control "  required>
                                ...
                                @foreach ($horarioLicencia as $horario)
                                    <option id="{{$horario->id}}" value="{{$horario->id}}" >{{$horario->descripcion}} </option>
                                @endforeach
                            </select>
                        </td>
                        <td style="width: 10%">
                    ...
                    @else
                        ...
                        <tr>
                            @if($loop->index == 0)
                                <td style="width: 35%"><input type="date" required class="form-control " id="dates[0][dateinicio]" name="dates[0][dateinicio]" onchange="activeDateFinish();" value="{{ old('fecha_inicio',$fecha->fecha_inicio) }}" value="{{ old('dates[0][dateinicio]') }}" ></td>
                                <td style="width: 35%"><input type="date" required class="form-control " id="dates[0][datefin]" name="dates[0][datefin]" onchange="activeDateFinish();" value="{{ old('fecha_inicio',$fecha->fecha_hasta) }}" value="{{ old('dates[0][datefin]') }}"></td>
                                <td style="width: 30%">
                                    <select id="dates[0][horario]" name="dates[0][horario]" class="form-control " required >
                                        <option value="">--   SELECCIONE   --</option>
                                        @foreach ($horarioLicencia as $horario)
                                            <option id="{{$horario->id}}" value="{{$horario->id}}"  {{ old("dates[][horario]",$fecha->horario_id) == $horario->id ? 'selected' : '' }}>{{$horario->descripcion}} </option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                            ...
                            @else                                
                                <td style="width: 35%"><input type="date" class="form-control " id="{{$fecha->id}}" name="dates[{{$fecha->id}}][dateinicio]" onchange="changeDate(id);" value="{{ old('fecha_inicio',$fecha->fecha_inicio) }}" ></td>
                                <td style="width: 35%"><input type="date" class="form-control " id="{{$fecha->id}}" name="dates[{{$fecha->id}}][datefin]" onchange="changeDate(id);" value="{{ old('fecha_inicio',$fecha->fecha_hasta) }}" ></td>
                                <td style="width: 30%">
                                    
                                    <select id="dates[{{$fecha->id}}][horario]" name="dates[{{$fecha->id}}][horario]" class="form-control " >
                                        <option value="">--   SELECCIONE   --</option>
                                        @foreach ($horarioLicencia as $horario)
                                            <option id="{{$horario->id}}" value="{{$horario->id}}" {{ old("dates[][horario]",$fecha->horario_id) == $horario->id ? 'selected' : '' }}>{{$horario->descripcion}} </option>
                                        @endforeach
